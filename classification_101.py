...
def myNaiveBayesTraining(trainSet):
    ...
    for t in trainSet:
        #last item in tuple is match or miss. 
        #TODO Assuming data is clean here
        if (t[1]=='miss'): 
            misses_counter = misses_counter+1
        else:
            matches_counter = misses_counter+1

        features = t[0]
        #http://stackoverflow.com/questions/4326658/python-index-a-dictionary
        #d.keys()[i]

        #for each feature in t(0), create frequency diagram. 
        #if the feature is in our counter, ++ it. Otherwise, add it with val 1. 

        for f in features:
            if(f in frequency_count):
                frequency_count[f]['total'] = frequency_count[f]['total'] +1
            else:
                frequency_count[f] = {'total':1,'miss':0,'match':0}

            #now add it to it miss or match data. 
            if(t[1]=='miss'):
                frequency_count[f]['miss'] = frequency_count[f]['miss']+1
            else:
                frequency_count[f]['match'] = frequency_count[f]['match']+1


    for f in frequency_count:
        print f
        print frequency_count[f]
